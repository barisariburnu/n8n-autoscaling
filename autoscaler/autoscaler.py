#!/usr/bin/env python3
"""
n8n Autoscaler Service

This service monitors the Redis queue length and dynamically scales n8n worker
containers using Docker Compose. It provides intelligent autoscaling based on
configurable thresholds and cooldown periods.

Features:
    - Dynamic worker scaling based on queue length
    - Support for multiple BullMQ queue patterns (v3, v4+, legacy)
    - Cooldown period to prevent scaling oscillations
...
COMPOSE_PROJECT_NAME = os.getenv('COMPOSE_PROJECT_NAME')  # e.g., "n8n-autoscaling"
...
def get_queue_length(r_conn):
    ...
    Supports multiple BullMQ queue patterns:
    ...
    try:
        ...
        logging.warning(
            f"Queue key patterns ('{key_to_check}', '{key_to_check_v4}', "
            f"'{key_to_check_legacy}') not found or not a list. Assuming length 0."
        )
        return 0
...
def get_current_replicas(docker_client, service_name, project_name):
    ...
    Args:
        ...
        service_name: Name of the service to count replicas for
        project_name: Docker Compose project name
        
    Returns:
        int: Number of running containers, or MAX_REPLICAS+1 on error
    """
    if not project_name:
    ...
    try:
        ...
        # Further filter by status in Python
        running_count = 0
        ...
        logging.info(
            f"Found {running_count} running containers for service '{service_name}' "
...
def scale_service(service_name, replicas, compose_file, project_name):
    ...
    try:
        ...
        logging.error(
            "docker compose command not found. Ensure it's installed in the "
            "autoscaler container and in PATH."
        )
        return False
...
def main():
    ...
    if not COMPOSE_PROJECT_NAME:
        logging.error(
            "CRITICAL: COMPOSE_PROJECT_NAME environment variable is not set. "
            "Autoscaler cannot function correctly."
        )
        logging.error(
            "Please set COMPOSE_PROJECT_NAME to the name of your Docker Compose project "
    ...
    logging.info(
        f"Autoscaler started. Monitoring n8n worker service '{N8N_WORKER_SERVICE_NAME}' "
        f"in project '{COMPOSE_PROJECT_NAME}'."
    )
    ...
    while True:
        try:
            ...
            if (current_time - last_scale_time) < COOLDOWN_PERIOD_SECONDS:
                remaining = COOLDOWN_PERIOD_SECONDS - (current_time - last_scale_time)
                logging.info(f"In cooldown period. Next check in {remaining:.0f}s.")
            ...
            if queue_len > SCALE_UP_QUEUE_THRESHOLD and current_reps < MAX_REPLICAS:
                ...
                logging.info(
                    f"Condition met for SCALE UP. Queue: {queue_len} > {SCALE_UP_QUEUE_THRESHOLD}. "
                    f"Replicas: {current_reps} < {MAX_REPLICAS}."
                )
            ...
            # Scale down logic
                ...
                logging.info(
                    f"Condition met for SCALE DOWN. Queue: {queue_len} < {SCALE_DOWN_QUEUE_THRESHOLD}. "
                    f"Replicas: {current_reps} > {MIN_REPLICAS}."
                )
                if scale_service(N8N_WORKER_SERVICE_NAME, new_replicas, COMPOSE_FILE_PATH, COMPOSE_PROJECT_NAME):
                    last_scale_time = current_time
                    scaled = True
